...
const PayTaxDue = ({ show, handleClose, propertyInfo, selectedPaymentDues, userData }) => {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            dues = await Promise.all(selectedPaymentDues.map(async due => await get(due)));
            dues.forEach(due => {
                const { basicQuarterTax, sefTax, discount, interest, subTotal, total } = due;
            ...
            setPeriodCovered(`${ quarter(latest.quarter, latest.year) } - ${ quarter(oldest.quarter, oldest.year) }`);
        })()
    }, [ selectedPaymentDues ])

    const submit = async () => {
        const {
            basicTax: basicQuarterTax,
            totalAmount: total,
            educationFund: sefTax,
            ...rest
        } = taxData;
        ...
        for (const due of selectedPaymentDues) {
            await update(due, { transactionId: transaction.id });
        }
      ...
      <Modal show={show} onHide={handleClose} size="lg" centered>
        ...
        <Modal.Body>
          <div className="container">
            ...
            <p className="fs-small font-italic pt-3 mb-0">
              *Please click the 'Pay via LandBank' button and copy  & paste the required
              information above.
            </p>
          </div>
        </Modal.Body>
        <Modal.Footer className="border-0">
          ...
          <Button variant="success" onClick={submit}>
            <strong>Pay via LANDBANK</strong>
          </Button>
        </Modal.Footer>
      </Modal>
    );
